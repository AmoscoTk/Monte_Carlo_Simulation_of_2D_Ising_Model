...
% Integer multiple for the number of spin updates per sweep
sweep_multiplier = 1;  % Modify this as needed
num_spin_updates_per_sweep = sweep_multiplier * L^2;
...
for ic = 1:length(initial_conditions)
    for t = 1:length(T_values)
        ...
        for sweep = 1:num_sweeps
            % Iterate over multiple spin updates per sweep
            for update = 1:num_spin_updates_per_sweep
                % Randomly select a spin
                linearIndex = randi(numel(lattice));
                [i, j] = ind2sub(size(lattice), linearIndex);
